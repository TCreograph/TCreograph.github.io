...
${u.blue("Please consider disable `cacheHTML` and `cachePic`, or set `maxSize` and `maxPicSize` option.\n")}`)})},d=(e,a)=>a.startsWith("/")?a.replace(/^\//,e):a,B=(e,a)=>(a.icons&&(a.icons=a.icons.map(n=>({...n,src:d(e,n.src)}))),a.shortcuts&&(a.shortcuts=a.shortcuts.map(n=>{var t;return{...n,icons:((t=n.icons)==null?void 0:t.map(o=>({...o,src:d(e,o.src)})))||[],url:d(e,n.url)}})),a.screenshots&&(a.screenshots=a.screenshots.map(n=>({...n,src:d(e,n.src)}))),a),H=(e,a)=>{a.favicon&&(a.favicon=d(e,a.favicon)),a.apple&&(a.apple.icon&&(a.apple.icon=d(e,a.apple.icon)),a.apple.maskIcon&&(a.apple.maskIcon=d(e,a.apple.maskIcon))),a.msTile&&a.msTile.image&&(a.msTile.image=d(e,a.msTile.image)),a.manifest&&(a.manifest=B(e,a.manifest))},L=(e,a="/",n=[])=>{var t;const o=[],c=[];n.forEach(l=>{l[0]==="meta"?o.push(l[1].name):l[0]==="link"&&c.push(l[1].rel)});let i="";const s=(l,p,g={})=>{c.includes(l)||n.push(["link",{rel:l,href:p,...g}])},r=(l,p,g={})=>{o.includes(l)||n.push(["meta",{name:l,content:p,...g}])};if(e.favicon&&s("icon",e.favicon),(t=e.manifest)!=null&&t.icons){const{icons:l}=e.manifest;l.length>0&&(i=l[0].src,e.manifest.icons.map(p=>{p.type?s("icon",p.src,{type:p.type,sizes:p.sizes}):s("icon",p.src,{sizes:p.sizes})}))}return s("manifest",`${a}manifest.webmanifest`,{crossorigin:"use-credentials"}),r("theme-color",e.themeColor||"#46bd87"),v(e.apple)&&(e.apple.icon||i)?(s("apple-touch-icon",e.apple.icon||i),r("apple-mobile-web-app-capable","yes"),r("apple-mobile-web-app-status-bar-style",e.apple.statusBarColor||"black"),e.apple.maskIcon&&s("mask-icon",e.apple.maskIcon,{color:e.themeColor||"#46bd87"})):e.apple!==!1&&i&&(s("apple-touch-icon",i),r("apple-mobile-web-app-capable","yes"),r("apple-mobile-web-app-status-bar-style","black")),v(e.msTile)&&(e.msTile.image||i)?(r("msapplication-TileImage",e.msTile.image||i),r("msapplication-TileColor",e.msTile.color||e.themeColor||"#46bd87")):e.msTile!==!1&&i&&(r("msapplication-TileImage",i),r("msapplication-TileColor",e.themeColor||"#46bd87")),r("viewport","width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no, viewport-fit=cover"),n},E=(e,a)=>{let n="",t="";return a.showInstall&&(n+=`import PWAInstall from "${f}components/PWAInstall.js";
`,t+=`PWAInstall,
...
`)},M=(e={},a=!1)=>n=>{a&&$(e),n.env.isDebug&&m.info("Options:",e);const{base:t,shouldPrefetch:o=!0}=n.options;e.appendBase&&H(t,e),o===!0&&m.warn('The plugin will register service worker to handle assets, so we recommend you to set "shouldPrefetch: false" in VuePress config file.');const c=A(n,e);return n.options.head=L(e,t,n.options.head),I(n,{id:"hope"}),{name:"vuepress-plugin-pwa2",define:()=>({PWA_LOCALES:S({app:n,name:"pwa",default:w,config:e.locales}),SW_FORCE_UPDATE:e.update==="force",SW_PATH:e.swPath||"service-worker.js"}),extendsBundlerOptions:(i,s)=>{C({app:s,config:i},["mitt","register-service-worker"]),x({app:s,config:i},{path:"/manifest.webmanifest",response:async(r,l)=>(l.setHeader("Content-Type","application/manifest+json"),JSON.stringify(await c)),errMsg:"Unexpected manifest generate error"})},onGenerated:async i=>{await W(i,c),await D(i,e)},clientConfigFile:i=>E(i,e)}};export{w as pwaLocales,M as pwaPlugin};
