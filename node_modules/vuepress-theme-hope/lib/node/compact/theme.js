...
export const convertThemeOptions = (themeOptions) => {
    // ensure plugins
    const plugins = (themeOptions["plugins"] ?? (themeOptions["plugins"] = {}));
    ...
    if (isPlainObject(themeOptions["navbarLayout"])) {
        if ("left" in themeOptions["navbarLayout"]) {
            logger.warn(`To have better meaning under RTL layout, ${colors.magenta("navbarLayout.left")}" option is deprecated, please use ${colors.magenta("navbarLayout.start")} instead`);
            themeOptions["navbarLayout"]["start"] = themeOptions["navbarLayout"]["left"];
        }
        if ("right" in themeOptions["navbarLayout"]) {
            logger.warn(`To have better meaning under RTL layout, ${colors.magenta("navbarLayout.right")}" option is deprecated, please use ${colors.magenta("navbarLayout.end")} instead`);
    ...
    if (isPlainObject(themeOptions["blog"]) && themeOptions["blog"]) {
        ...
        if (!plugins["blog"])
            logger.warn(`Blog feature is tree-shakable in v2, you should set ${colors.magenta("plugins.blog: true")} in theme options to enable it.`);
    }
    // handle component
    if (isArray(plugins["components"])) {
        logger.warn(`${colors.magenta("plugins.components")} no longer accepts array, please set it to ${colors.magenta("plugin.components.components")} instead.`);
        plugins["components"] = {
            components: plugins["components"],
        };
    }
    // handle copyright plugin
    if (isPlainObject(themeOptions["copyright"]) ||
        typeof themeOptions["copyright"] === "boolean") {
        logger.warn(`${colors.magenta("copyright")} is deprecated in V2, please use ${colors.magenta("plugins.copyright")} instead.`);
    }
    // handle addThis
    ...
    if (isPlainObject(themeOptions["encrypt"]) && themeOptions["encrypt"]) {
        ...
        if ("global" in encrypt && typeof encrypt["global"] !== "boolean") {
            logger.warn(`${colors.magenta("encrypt.global")} is deprecated in V2, please use ${colors.magenta("encrypt.admin")} instead.`);
            encrypt["admin"] = encrypt["global"];
        }
        if ("status" in encrypt) {
            logger.warn(`${colors.magenta("encrypt.status")} is deprecated, please use ${colors.magenta("encrypt.global")} instead.`);
    ...
    if ("locales" in themeOptions && isPlainObject(themeOptions["locales"])) {
        values(themeOptions["locales"]).forEach((localeConfig) => {
            DEPRECATED_THEME_OPTIONS.forEach(([deprecatedOption, newOption]) => deprecatedLogger({
