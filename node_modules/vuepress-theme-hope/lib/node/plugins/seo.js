...
export const getSEOPlugin = (themeData, { seo }, hostname = "", legacy = true) => {
    if (seo === false)
        return null;
    const seoOptions = isPlainObject(seo) ? seo : {};
    // disable seo if `hostname` is not set and no options for seo plugin
    if (!keys(seoOptions).length && !hostname)
        return null;
    return seoPlugin({
        hostname,
        ...(themeData.author ? { author: themeData.author } : {}),
        isArticle: ({ filePathRelative, frontmatter }) => Boolean(filePathRelative) &&
            !frontmatter["home"] &&
