 ...
 */
export const checkPlugins = (app, plugins) => {
    PLUGIN_CHECKLIST.forEach(([pluginName, optionName = ""]) => {
        if (app.pluginApi.plugins.some(({ name }) => name === pluginName))
            logger.error(`Plugin "${colors.magenta(pluginName)}" is used by theme, so you are not allowed to call this plugin yourself in ${colors.cyan("vuepress config file")} . ${optionName
                ? `Set "${colors.magenta(`plugin.${optionName}`)}" in ${colors.cyan("theme options")}, to customize it.`
                : ""}`);
    });
    keys(plugins).forEach((key) => {
        if (!KNOWN_THEME_PLUGINS.includes(key))
            logger.warn(`You are setting "${colors.magenta(`plugins.${key}`)}" option in ${colors.cyan("theme options")}, but it's not supported by theme. You need to install the plugin yourself and import then call it manually in "${colors.magenta("plugins")}" options in ${colors.cyan("vuepress config file")} directly.`);
    });
};
/**
